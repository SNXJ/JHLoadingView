...
public class LoadingView extends View {
    ...
    private boolean isRecovery = false;//是否恢复原来状态
    ...
    private void loadAttrs(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.loading_jh);
        paintBold = array.getDimensionPixelSize(R.styleable.loading_jh_paintBold, 15);
        lineLength = array.getDimensionPixelSize(R.styleable.loading_jh_lineLength, 35);
        bgPaintColor = array.getColor(R.styleable.loading_jh_backgroundColor, Color.WHITE);
        beforePaintColor = array.getColor(R.styleable.loading_jh_beforeColor, Color.YELLOW);
        lines = array.getInt(R.styleable.loading_jh_lines, 20);
        max = array.getInt(R.styleable.loading_jh_max, 100);
        progress = array.getInt(R.styleable.loading_jh_progress, 0);
        textColor = array.getColor(R.styleable.loading_jh_textColor, Color.WHITE);
        array.recycle();
    }
    ...
    private void initPaint() {
        ...
        bfPaint.setStrokeJoin(Paint.Join.MITER);
        bfPaint.setStrokeCap(Paint.Cap.SQUARE);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        progress = (progress == 100 && isRecovery) ? 0 : progress;
...
//        canvas.drawText("审核中", x, y + 5, textPaint);
        //绘制上层菊花,也就是进度
        canvas.rotate(360 / lines, x, y);
        for (; count > 0; count--) {
     ...
     * @param duration 动画时长
     */
    public void startAnimation(final int start, final int current, final int duration) {
        ...
        progressAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (progress == 100 && isLoop) {
                    startAnimation(start, current, duration);
                }
                invalidate();
            }
